...
- **comment-check** — Read-only comment-discipline gate (`/comment-check`). The check-only sibling of `no-comments`: dispatches a hostile `general-purpose` critic (persona in `references/comment-critic.md`) over a scope, then reports its `COMMENT-CHECK: PASS`/`FAIL` verdict verbatim. Flags narration, workaround sermons, commented-out corpses, and unsafe lint/TS suppressions; never edits code, so it works as a critic quality gate.
