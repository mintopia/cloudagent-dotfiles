...
#   - settings-hooks.jq   (wires the cloudagent-skill hook)
...
ok "idempotent: no duplicate SessionStart cmds" 'test "$(printf "%s" "$OUT" | jq "[.hooks.SessionStart[].hooks[].command]|length")" -eq 1'
