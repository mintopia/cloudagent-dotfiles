...
Before spawning investigators, list the available MCPs in your environment. Use your available tool list and tool namespaces to see which MCP servers are enabled.
...
Subagent config (each): spawn a Claude subagent via the Agent/Task tools.
- `subagent_type`: `general-purpose`
- Model: a fast tier is fine. Investigators run in parallel and each does focused retrieval, not deep synthesis.
- Give each subagent full tool access. **Do not restrict it to a read-only tool set.** That strips MCP access, which disables MCP-backed investigators entirely. The source control investigator would be safe read-only, but keep the posture uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent via the Agent/Task tools:

- `subagent_type`: `general-purpose`
- Model: a capable model. Synthesis is the quality-critical step. For a genuine second-model pass — an independent cross-check from a different model family than the investigators — run the synthesis through the codex agent/CLI.
- Give it full tool access. The synthesizer's quality check spot-verifies citations, which can require MCP access; a read-only tool set strips MCPs and defeats that.
