...
- **These levers trade memory for speed.** Reusing workers and running files concurrently keeps more suites resident at once; on a constrained box that is what OOM-kills a "faster" config mid-run. Cap it (`poolOptions.threads.maxThreads`, `--maxWorkers`) and watch peak RSS — see *Speed has a ceiling* in [SKILL.md](SKILL.md).
