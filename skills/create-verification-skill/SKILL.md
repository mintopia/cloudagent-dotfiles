...
- **Drive:** how can an agent interact with it programmatically? Existing harnesses first — Playwright/Cypress specs, expect scripts, PTY helpers, curl-able endpoints, a debug port. Only then pick a generic recipe: for web and Electron the default driver is the **playwright MCP/CLI** (it drives the real UI and gives you ARIA-based handles for free); a tmux/PTY harness for CLI/TUI; plain HTTP for services. The recipe stays language- and platform-agnostic — playwright is just the go-to for anything with a browser/UI surface.
...
- **Drive:** the harness recipe with real selectors/commands from this repo, not examples. For a web/UI surface, default to the playwright MCP/CLI and prefer stable handles (ARIA labels, data attributes, prompt strings, route paths) over coordinates and tab order.
